...
() => {
    ...
    for (const sel of shipSels) {
        try {
            ...
            if (el) {
                ...
                // Check for a price FIRST — the element may contain both a price
                // AND promo text like "Buy 12 and get free shipping", so we must
                // extract the actual cost before checking for the word "free".
                ...
                // Check for "free shipping" — but IGNORE conditional promo text
                // like "Buy 12 and get free shipping" (that's a bulk-buy promo,
                // the actual shipping cost is shown separately).
                if (sLower === 'free' || sLower === 'free shipping') {
                    result.shipping = 'Free';
                    break;
                }
                if (/free\\s+shipp/i.test(sText) && !/buy\\s+\\d+.*free\\s*shipp/i.test(sText)) {
                    result.shipping = 'Free';
                    break;
                }
    ...
    // Strategy 3: Look for "Shipping: £X.XX" or "Shipping: Free" in page text
    if (!result.shipping) {
        const bodyText = document.body.innerText || '';
        const shipMatch = bodyText.match(/[Ss]hipping[:\\s]*[£$€]\\s*([\\d,]+\\.\\d{2})/);
        if (shipMatch) {
            result.shipping = shipMatch[1].replace(',', '');
        } else if (/[Ss]hipping[:\\s]*[Ff]ree/i.test(bodyText)) {
            // Only match "Shipping: Free" or "Shipping Free" — not random "free shipping" promo text
            result.shipping = 'Free';
        }
    }
...
def _force_jpg_extension(url):
    """Force URL to use .jpg extension — Amazon rejects .webp."""
    if not url:
        return url
    url = re.sub(r'\.webp(\?.*)?$', r'.jpg\1', url, flags=re.IGNORECASE)
    url = re.sub(r'\.png(\?.*)?$', r'.jpg\1', url, flags=re.IGNORECASE)
    return url
    ...
    if offer_col_indices:
        ...
        with open(offer_name, "w", encoding="utf-8") as f:
            ...
            for sku in sorted(offer_sku_prices):
                ...
                f.write(f"toyfigure\t{sku}\tPartialUpdate\tNew\t{price}\tDEFAULT\t{QUANTITY}\t{HANDLING_DAYS}\t{price}\n")
