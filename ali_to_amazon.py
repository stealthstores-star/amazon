...
() => {
    ...
    for (const sel of shipSels) {
        try {
            ...
            if (el) {
                ...
                // --- IMPORTANT: Check for "Free" FIRST ---
                // AliExpress often shows a struck-through old price next to "Free",
                // e.g. "£8.00  Free" or "Shipping: £8.00 → Free".
                // If we check price first, we'd grab the old £8.00 incorrectly.

                // Exact match: element text is just "Free" or "Free shipping"
                if (sLower === 'free' || sLower === 'free shipping') {
                    result.shipping = 'Free';
                    break;
                }

                // "Free" appears prominently but NOT as conditional promo
                // Reject: "Buy 12 and get free shipping", "Free shipping on orders over £30",
                //         "free shipping for orders above", "spend £X for free shipping"
                const isConditionalFree = /buy\\s+\\d+.*free\\s*shipp/i.test(sText)
                    || /orders?\\s+(over|above)/i.test(sText)
                    || /spend\\s+[£$€]/i.test(sText)
                    || /for\\s+orders/i.test(sText);

                if (/free\\s*shipp/i.test(sText) && !isConditionalFree) {
                    result.shipping = 'Free';
                    break;
                }
                // Also catch standalone "Free" near shipping context (not conditional)
                if (/\\bfree\\b/i.test(sText) && !isConditionalFree && sText.length < 80) {
                    result.shipping = 'Free';
                    break;
                }

                // --- Now check for actual shipping price ---
                const sMatch = sText.match(/[Ss]hipping[:\\s]*[£$€¥₽]\\s*([\\d,]+\\.?\\d*)/);
                if (sMatch) {
                    const val = parseFloat(sMatch[1].replace(',', ''));
                    result.shipping = val === 0 ? 'Free' : sMatch[1].replace(',', '');
                    break;
                }
                // Match any currency: £3.52, $2.50, €1.80, etc.
                const sMatch2 = sText.match(/[£$€¥₽]\\s*([\\d,]+\\.?\\d*)/);
                if (sMatch2) {
                    const val = parseFloat(sMatch2[1].replace(',', ''));
                    result.shipping = val === 0 ? 'Free' : sMatch2[1].replace(',', '');
                    break;
                }
                // Also match "3.52" without currency symbol
                const sMatch3 = sText.match(/(\\d+[,.]\\d{2})/);
                if (sMatch3 && sLower.includes('ship')) {
                    result.shipping = sMatch3[1].replace(',', '');
                    break;
                }
            }
        } catch(e) {}
    ...
    // Strategy 3: Look for "Shipping: Free" or "Shipping: £X.XX" in page text
    if (!result.shipping) {
        const bodyText = document.body.innerText || '';
        // Check "Shipping: Free" FIRST (before price) to avoid old struck-through prices
        if (/[Ss]hipping[:\\s]*[Ff]ree/i.test(bodyText)) {
            result.shipping = 'Free';
        } else {
...
def _download_and_convert(img_url):
    ...
    if not has_pillow:
        log.warning("Pillow not installed — cannot convert to JPEG, skipping image")
        return None
    ...
    try:
        ...
        log.warning("Failed to convert image to JPEG: %s — %s", img_url[:80], e)
        return None
...
def _force_jpg_extension(url):
    """Force URL to use .jpg extension — Amazon rejects .webp, .gif, .bmp etc."""
    if not url:
        return url
    # Replace any non-jpg image extension with .jpg
    url = re.sub(r'\.(webp|png|gif|bmp|tiff|avif)(\?.*)?$', r'.jpg\2', url, flags=re.IGNORECASE)
    return url
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            url = (img_data.get("image", {}).get("url", "")
                   ...
                   or img_data.get("url", ""))
            if url:
                return _force_jpg_extension(url)
    except Exception:
        pass
    return None
...
def _upload_to_imgur(jpeg_bytes):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if link:
                return _force_jpg_extension(link.replace("http://", "https://"))
    except Exception:
        pass
    return None
...
def rehost_image(img_url):
    ...
    # Try freeimage first (most reliable, no rate limit issues)
...
def ali_to_gbp(price_val, shipping_str=None):
    """Calculate Amazon UK sell price from AliExpress cost.

    AliExpress shows prices in the user's local currency (GBP for UK users),
    so the scraped price is already in GBP — no currency conversion needed.
    Shipping is also already in GBP.
...
def make_description(title, ali_description=None):
    ...
    if ali_features:
        ...
        if clean_features:
            sections.append("<br>".join(clean_features))
    ...
    if spec_items:
        sections.append("Specifications:<br>" + "<br>".join(spec_items[:10]))
...
def find_amazon_template():
    exclude = ["upload", "processing", "report", "broken", "summary", "offer", "fix_combined", "draft"]
    xlsm_files = [f for f in os.listdir(".")
                  if f.endswith(".xlsm") and not any(ex in f.lower() for ex in exclude)]
    if not xlsm_files:
        return None
...
def _load_all_existing_sku_prices():
    """Load SKU->GBP sell price from all existing upload/offer/CSV files."""
    import glob as _glob
    prices = {}

    # From CSVs (lowest priority — recalculate from source price)
    for pat in ['aliexpress_scrape*.csv', '*resin_models.csv']:
        for csv_path in sorted(_glob.glob(pat), key=os.path.getmtime, reverse=True):
            try:
    ...
    # From .txt files (higher priority — already have correct GBP sell prices)
    for pattern in ['amazon_upload_*.txt', 'amazon_offer_update_*_FIXED.txt',
                     'amazon_offer_update_*_OFFER.txt', 'amazon_offer_update_*.txt']:
        for tf in sorted(_glob.glob(pattern), key=os.path.getmtime, reverse=True):
            if 'processing' in tf:
                continue
            try:
                ...
                for i, h in enumerate(headers):
                    ...
                    if hl == 'item_sku':
                        sku_col = i
                    elif 'our_price' in hl and 'a1f83g8c2aro7p' in hl:
                        price_col = i
                    elif hl == 'list_price_with_tax' and price_col is None:
                        price_col = i
                if sku_col is None or price_col is None:
                    continue
                for line in lines[3:]:
                    parts = line.strip().split('\t')
                    if len(parts) <= max(sku_col, price_col):
                        continue
                    sku = parts[sku_col].strip()
                    pv = parts[price_col].strip()
                    if sku.startswith('ALI-') and pv:
                        try:
                            prices[sku] = float(pv)
                        except ValueError:
                            pass
...
def fill_amazon_template(template_path, products):
    ...
    for product in products:
        ...
        # Skip products with no rehosted images — Amazon rejects listings without a main image
        if not images:
            log.warning("  SKIP no images: %s", raw_title[:80])
            continue
    ...
    for r in range(start_row, start_row + filled):
        ...
        for c in valid_cols:
            ...
            # Replace newlines/tabs with spaces — newlines break tab-delimited format
            cell = str(val).replace("\n", " ").replace("\r", " ").replace("\t", " ") if val is not None else ""
    ...
    for f in offer_fields_needed:
        ...
        if c:
            ...
            offer_fields_found.append(f)
    if offer_col_indices:
        # Row 1: use metadata from best available source.
        # Priority: 1) *_FIXED.txt  2) amazon_upload_*.txt  3) template metadata
        import glob as _glob
        offer_r1 = None

        fixed_files = sorted(_glob.glob('amazon_offer_update_*_FIXED.txt'),
                             key=os.path.getmtime, reverse=True)
        if fixed_files:
            with open(fixed_files[0], 'r', encoding='utf-8') as fref:
                ref_row1 = fref.readline().rstrip('\n')
                offer_r1 = ref_row1.split('\t')
                offer_r1 = (offer_r1 + [''] * len(offer_col_indices))[:len(offer_col_indices)]
            log.info("  Offer row1 metadata from: %s", fixed_files[0])

        if not offer_r1:
            # Try upload files or offer update files
            for pattern in ['amazon_upload_*.txt', 'amazon_offer_update_*.txt']:
                ref_files = sorted(_glob.glob(pattern), key=os.path.getmtime, reverse=True)
                for rf in ref_files:
                    if 'processing' in rf:
                        continue
                    try:
                        with open(rf, 'r', encoding='utf-8') as fref:
                            ref_row1 = fref.readline().rstrip('\n')
                        offer_r1 = ref_row1.split('\t')
                        offer_r1 = (offer_r1 + [''] * len(offer_col_indices))[:len(offer_col_indices)]
                        log.info("  Offer row1 metadata from: %s", rf)
                        break
                    except Exception:
                        continue
                if offer_r1:
        ...
        if not offer_r1:
            log.warning("  No reference file found for row1 metadata, using template metadata (may fail)")
            meta_parts = []
            for mc in sorted(ws_copy_r1.keys()):
                v = ws_copy_r1.get(mc)
                if v and str(v).strip():
                    s = str(v).strip()
                    if s.startswith(("TemplateType=", "Version=", "TemplateSignature=", "settings=")):
                        meta_parts.append(s)
            offer_r1 = (meta_parts + [''] * len(offer_col_indices))[:len(offer_col_indices)]
        # Row 2 and 3: pick the correct display name / attribute name for each offer column
        offer_r2 = [str(ws_copy_r2.get(c) or "") for c in offer_col_indices]
        offer_r3 = [str(ws_copy_r3.get(c) or "") for c in offer_col_indices]
        # Collect current batch SKU->price from worksheet data
        sku_col_idx = None
        price_col_idx = None
        for ci, field in enumerate(offer_fields_found):
            if field == 'item_sku':
                sku_col_idx = offer_col_indices[ci]
            elif 'our_price' in field:
                price_col_idx = offer_col_indices[ci]
            elif field == 'list_price_with_tax' and price_col_idx is None:
                price_col_idx = offer_col_indices[ci]

        # Build SKU->price map: start with current batch
        offer_sku_prices = {}
        for r in range(start_row, start_row + filled):
        ...
        for sku, price in existing_prices.items():
            if sku not in offer_sku_prices:
                offer_sku_prices[sku] = str(price)

        log.info("  Offer file: %d from current batch + %d from previous files = %d total",
                 filled, len(offer_sku_prices) - filled, len(offer_sku_prices))
        ...
        with open(offer_name, "w", encoding="utf-8") as f:
            ...
            for sku in sorted(offer_sku_prices):
                price = offer_sku_prices[sku]
                row_vals = []
                for field in offer_fields_found:
                    fl = field.lower()
                    if fl == 'item_sku':
                        row_vals.append(sku)
                    elif 'our_price' in fl or fl == 'list_price_with_tax':
                        row_vals.append(str(price))
                    elif field in offer_field_values:
                        row_vals.append(offer_field_values[field])
                    else:
                        row_vals.append('')
                f.write("\t".join(row_vals) + "\n")
        log.info("  Also generated offer update file: %s (%d SKUs)", offer_name, len(offer_sku_prices))
...
def post_process(csv_path):
    ...
    if not template_path:
        ...
        log.info("  Place the Amazon TOY_FIGURE.xlsm template here and re-run.")
        log.info("  Scraped CSV: %s", csv_path)
        return
        ...
        if args.titles_only:
            ...
            () => {
                ...
                for (const sel of titleSels) {
                    const el = document.querySelector(sel);
                    if (el && el.innerText.trim().length > 5) { title = el.innerText.trim(); break; }
                }
                ...
                return { title, price, image, images: [...new Set(images)] };
            }
            """
                ...
                if not args.skip_details and products:
                    ...
                    if detail_results:
                        for p_idx, product in enumerate(products):
                            ...
                            # Always prefer the detail page price — it's more accurate
                            # and guaranteed to be in the user's local currency (GBP)
